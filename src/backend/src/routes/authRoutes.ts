...
router.put('/profile', protect, validate([body('name').optional().trim().notEmpty().withMessage('Name cannot be empty')]), authController.updateProfile);
router.put('/password', protect, validate([body('currentPassword').notEmpty().withMessage('Current password is required'), body('newPassword').isLength({
  min: 6
